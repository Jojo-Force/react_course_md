...
![image-20251229131635301](1.assets/image-20251229131635301.png)



* 查看渲染了几次

  ![image-20251229132004955](1.assets/image-20251229132004955.png)

* 使用children传递子组件可以减少重新渲染的次数，当父组件变化时，子组件如果没有更变，子组件不会重新渲染。
...
* memoization:每当执行纯函数后将结果存放在内存中，如果再次执行函数使用相同的输入，那么结果一定是相同的，使用之前的结果返回，这样就不用再执行一次函数。
  * 组件memo:使用memo
  * 对象memo:使用useMemo
  * 函数memo：使用useCallback

![image-20251229143556950](1.assets/image-20251229143556950.png)



* memo的作用: 如果父组件更新，但是传给子组件的参数没有变，那子组件不会更新
* 对于渲染过慢的组件或者经常渲染的组件有用。

![image-20251229143616243](1.assets/image-20251229143616243.png)
